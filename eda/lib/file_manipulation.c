...
//Função para montar o caminho do arquivo de acordo com o número aleatório
...
//Monta vetor de números aleatórios para escolher os arquivos
void chose_files(int *photos, int *rest){
  ...
  //Montando vetor de números restantes
  k=0;
  while(k < 25){
    for(i = 1; i < 51; i++){
      unique = 1;
      for(j=0; j<25; j++){
        //Verifica s o número já foi escolhido
        if(photos[j] == i){
          unique = 0;
        }
      }
      //Se o número não estiver no vetor de números aleatórios ele é adicionado
      //ao vetor de números restantes
      if(unique){
        rest[k] = i;
        k++;
      }
